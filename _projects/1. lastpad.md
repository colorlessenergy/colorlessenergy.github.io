...
meta: a PWA to access notes offline
...
If the `navigator.onLine` was true it would just make a fetch request otherwise if it was false or the fetch failed it would try to make the operation offline using either using the arrays in localStorage.

I have a JavaScript timer that runs every 5 seconds to check if the user went back online and there was changed made offline to send requests to the backend to create the new changes in MongoDB.
